...
const ArticleSectionCard = ({ article }) => {
  let blogCategory =
    article?.attributes?.Blog_category?.data?.attributes?.Title?.toLowerCase()
      ?.split(" ")
      ?.join("-");
  let blogTitle = encodeURIComponent(
    article?.attributes?.Blog_title?.toLowerCase()?.split(" ")?.join("-")
  );
  return (
    <Link
      to={`/articulos/${blogCategory}/${blogTitle}`}
      className="flex justify-center items-center w-full sm:w-full md:w-full lg:w-full relative"
    >
      <LazyLoadImage
